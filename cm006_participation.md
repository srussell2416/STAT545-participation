...
#This is a form of "self-documenting"
gapminder %>%
...
```r
gapminder %>%
  mutate(gdpBill = (pop*gdpPercap/1e9) %>% round(digits = 2))
```
...
```r
gapminder %>%
  transmute(GDP = (pop*gdpPercap) %>% round(digits = 2))
```
...
```
## Error in is_character(x): object 'gdpPErcap' not found
```
}}
